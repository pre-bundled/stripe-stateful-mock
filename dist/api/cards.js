...
(function (cards) {
    const cardExtras = {};
    function createFromSource(token) {
        log.debug("cards.createFromSource", token);
        let saveCard = true;
        const cardId = `card_${utils_1.generateId(24)}`;
        const now = new Date();
        const card = {
            id: cardId,
            object: "card",
    ...
    cards.createFromSource = createFromSource;
    function getCardExtra(cardId) {
        return cardExtras[cardId];
    }
    cards.getCardExtra = getCardExtra;
